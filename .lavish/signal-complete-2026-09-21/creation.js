...
(function () {
  ...
  window.renderCreation = function renderCreation(route, ctx) {
    if (!['createType', 'createActivity', 'createDates', 'createZone', 'createAmount', 'createReview', 'createSaved'].includes(route)) return null;
    ...
    const rules = () => [
      ...
      p('An assigned reviewer can inspect the challenge details needed for your review. Raw Apple Health records are not shared.'),
    ...
    const stepNames = ['createType', 'createActivity', 'createReview'];
    const direct = Boolean(state.createDirect);
    const stepNumber = stepNames.indexOf(route) + (direct ? 0 : 1);
    const progress = route === 'createSaved' || ['createDates','createZone','createAmount'].includes(route) ? '' : '<p class="eyebrow">Step ' + stepNumber + ' of ' + (direct ? 2 : 3) + '</p>';
    ...
    if (route === 'createActivity') {
      const choices = Object.entries(metrics).map(([id, item]) => '<button class="activity-choice" data-action="create-metric:'+id+'" aria-pressed="'+(d.metric===id)+'">'+esc(item.title)+'</button>').join('');
      let content = heading('Goal & dates') + '<div class="activity-selector">'+choices+'</div>';
      if (unavailable) content += notice('Activity not available yet. Choose another activity or check again later.');
      if (d.metric === 'timed') content += field('Whole run', 'distance', d.distance, 'kilometres', 'inputmode="decimal" placeholder="5" aria-label="Whole run in kilometres"');
      if (personal) {
        content += '<section class="fields goal-entry">' + field(d.metric === 'timed' ? 'Time to beat' : 'Your goal', 'target', d.target, metric.unit, 'inputmode="' + (d.metric === 'steps' ? 'numeric' : d.metric === 'distance' ? 'decimal' : 'text') + '" placeholder="' + metric.example + '" aria-label="' + esc(d.metric === 'timed' ? 'Time to beat in minutes and seconds' : metric.unit) + '"') + '</section>';
        content += disclosure('Use a suggestion', state.readiness==='ready' ? p('Suggestion: ' + metric.suggested + '. You choose whether to use this goal.') + button('Use this suggestion', 'create-suggest:' + metric.suggestion, 'secondary') : p('We need matching activity before suggesting a goal. You can enter your own.'));
      } else content += '<div class="neutral-statement"><h2>'+ (leaderboard?'Compare saved activity':'Each person sets a goal')+'</h2><p>'+(leaderboard?'No target is needed. Everyone agrees to the same activity and dates.':'Invite your friends next. Everyone chooses their own target before agreeing.')+'</p></div>';
      content += '<div class="editor-rows">'+ctx.row('Dates',duration ? duration+' full days · '+zone:'Choose your dates',duration ? day(first,false)+' – '+day(last,false):'Choose','createDates')+
        ctx.row('Simulated amount','Fee $0 · No real money moves',amount,'createAmount')+'</div>';
      if(d.metric==='minutes')content+=disclosure('About Activity minutes',p(source));
      return content + next('Review');
    }

    if (route === 'createDates') return heading('Dates') +
      '<div class="fields">' + field('Duration', 'days', d.days, 'full days · choose 1–30', 'inputmode="numeric" aria-label="Duration in full days"') +
      '<label class="field"><span>Starts</span><input type="date" data-field="draft.start" value="' + esc(d.start) + '" min="2026-09-23" max="2026-10-21"></label></div>' +
      ctx.row('Time zone','Dates stay in this time zone',zone,'createZone') +
      (duration ? range() : '') + p('Start in 2–30 days; each day runs midnight to midnight.') +
      (duration ? disclosure('Exact start and end', boundary()) : '') + '<div class="actions">'+button('Done','editor-done')+'</div>';

    if (route === 'createZone') return heading('Time zone')+'<label class="field"><span>Search</span><input type="search" placeholder="City or time zone" id="zone-search"></label>'+
      '<label class="field"><span>Time zone</span><select data-field="draft.zone">' + Object.entries(zones).map(([id, title]) => '<option value="' + esc(id) + '"' + (d.zone === id ? ' selected' : '') + '>' + esc(title) + '</option>').join('') + '</select></label>' +
      p('Your challenge keeps these dates if you travel.')+'<div class="actions">'+button('Done','editor-done')+'</div>';

    if (route === 'createAmount') return heading('Simulated amount') + '<div class="fields">' +
      field('Amount · $', 'amount', d.amount, 'USD · $1–$500', 'inputmode="numeric" aria-label="Simulated amount in whole US dollars"') + '</div>' +
      p('Simulated stakes — no real money moves.') + facts([['Fee', '$0'], ['Real money', '$0']]) + '<div class="actions">'+button('Save amount','editor-done')+'</div>';
