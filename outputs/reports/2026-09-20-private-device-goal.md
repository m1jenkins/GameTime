...
This private device slice is not wider Beta readiness. The four friend leaderboard
modes and other source, operational, human and release requirements remain
unresolved for hosted release. Concurrent local `main` commits added D141's
received-score leaderboard implementation and a Signal design plan while this
device work was running; the merge preserves them. Their additional migration
was not deployed to the selected backend, and leaderboard mode is not offered
by this steps-only build. D140's distribution-scope decision is unchanged.
The accepted phone executable corresponds to private milestone commit `8bf05dd`
(apart from comment/documentation-only changes); it was not replaced with the
separately advanced `main` build during merge verification.
Merged-source verification ran 38 focused native checks: 37 passed initially,
and one leaderboard render assertion failed because the OCR transcript inserted
repeated navigation/banner text into a sentence split across scroll pages.
The captured screenshots visibly contained both the full deadline sentence and
Create lobby. Normalizing only that repeated chrome in the assertion fixed the
test, which then passed on a focused rerun; no product change was needed.
The merged check's result bundles are `native-private-main-merge.xcresult` and
`native-private-main-merge-ocr.xcresult` under the same task temporary directory.
Its separate disposable simulator was removed after verification.
