...
def main() -> None:
    ...
    try:
        ...
        try:
            ...
            held.release()
        results = [process.communicate(timeout=20) for process in contenders]
        check(all(process.returncode == 0 for process in contenders) and sum("challenge_link_unavailable" in output for output, _ in results)==1, "same-link race admits exactly the twentieth account")
        ...
        output, error = revoke_waiter.communicate(timeout=20)
        check(revoke_waiter.returncode == 0 and "challenge_link_unavailable" in output, 'revoked link rejects waiting redemption')
        ...
        sql(f"select public.challenge_grant_support_v1('{actors[47]}','2026-10-05T00:00Z'); select public.challenge_readiness_fixture_v1('{actors[37]}')")
        ...
        held = Held(f"select set_config('request.jwt.claims',{literal(json.dumps({'sub':actors[47],'session_id':sessions[47]}))},true); set local role authenticated; select public.challenge_support_suspend_v1('{uuid.uuid4()}','{actors[37]}','unsafe_behavior')")
        ...
        results = [p.communicate(timeout=20) for p in contenders]
        check(all(p.returncode!=0 for p in contenders) and '42501' in results[0][1] and 'P0001' in results[1][1],
        ...
        # A grant revoked while a report read waits must not authorize that read.
        held = Held(f"select 1 from app.challenge_operator_grants_v1 where actor_id='{actors[47]}' and challenge_id='{community}' and capability='moderate' for update")
        name = 'p6_grant_revoke_' + uuid.uuid4().hex
        reader = contender(auth(actors[47],sessions[47],f"select public.challenge_operator_reports_v1('{community}')"),name)
        try: check(blocked([name]),'moderator read waits on its exact grant row')
        finally: held.release(f"select set_config('app.challenge_write_v1','on',true);update app.challenge_operator_grants_v1 set expires_at=app.challenge_now_v1() where actor_id='{actors[47]}' and challenge_id='{community}' and capability='moderate'")
        _,error = reader.communicate(timeout=20)
        check(reader.returncode!=0 and '42501' in error,'revoked grant rejects waiting report read')
        sql(f"select public.challenge_grant_operator_v1('{actors[47]}','{community}','moderate','2026-10-05T00:00Z')")
