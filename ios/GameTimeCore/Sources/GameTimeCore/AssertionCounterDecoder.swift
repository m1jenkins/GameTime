...
public enum AssertionCounterDecoder {
    public static func decode(from assertion: Data) throws -> UInt32 {
        guard !assertion.isEmpty, assertion.count <= 64 * 1024 else {
...
private enum CounterDecodingError: Error {
    case truncated
...
private struct CBORReader {
    ...
    mutating func readContainerCount(
        ...
        guard let count = Int(exactly: head.argument), count <= bytes.count else {
            throw CounterDecodingError.excessiveLength
        }
        return count
    }
