...
@MainActor
final class ChallengeHealthAppleWorkoutReaderTests: XCTestCase {
  func testInProgressWindowCapsTheReadButPreservesTheFrozenRequest() async throws {
    let now = Date()
    let fake = WorkoutQueryFake(records: [workout(start: now.addingTimeInterval(-60), end: now.addingTimeInterval(-30))])
    let reader = ChallengeHealthAppleWorkoutReader(querying: fake)
  ...
  func testAnchoredDeletionAndAnOrdinaryDisappearanceRemainDistinct() {
    let record = workout()
    var ledger = ChallengeHealthAppleWorkoutLedger()
  ...
  private func request(actor: UUID, start: Date, end: Date) throws -> ChallengeHealthReadRequest {
    ...
    let binding = try ChallengeHealthBinding(actorID: actor, challengeID: id(9), agreementVersion: 1,
      termsDigest: String(repeating: "a", count: 64), metric: .runningMillimeters, challengeWindow: challenge,
      realSourcePolicy: .appleWorkoutOutdoorDistanceV1)
  ...
  private func workout(start: Date = Date(timeIntervalSince1970: 1_800_000_000),
                   end: Date = Date(timeIntervalSince1970: 1_800_000_060)) -> WeeklySourceRecord {
    return WeeklySourceRecord(id: id(10), metric: .runningDistanceMillimeters, start: start, end: end, value: 1_000,
...
@MainActor
private final class WorkoutQueryFake: ChallengeHealthAppleWorkoutQuerying {
  ...
  private var pending: CheckedContinuation<ChallengeHealthAppleWorkoutBoundedRecords, Error>?
  ...
  func boundedWorkouts(in interval: DateInterval) async throws -> ChallengeHealthAppleWorkoutBoundedRecords {
    ...
    return .init(records: records, wasTruncated: wasTruncated)
  }
  ...
  func resumeFirstBoundedRead() {
    let value = ChallengeHealthAppleWorkoutBoundedRecords(records: records, wasTruncated: wasTruncated)
