            ...
            if store.actorID == nil {
                ...
                if let draft {
                    Section("Review before agreeing") {
                        ...
                        Toggle("I agree to save these rules as a private fictional practice record.", isOn: $consent)
...
private struct MetricPrototypeRules: View {
    ...
    var body: some View {
        ...
        if draft.format == .timedDistance {
            ...
            Text("Each update replaces your previous total for these dates. We do not add the updates together or use a total to decide how fast you ran.")
        }
        Text("You may replace an update, including lowering it or marking data missing, until strictly before 48 hours after this window ends. Missing data never means failure. There is no final result or review process in this practice tool.")
        Text("You can leave at any time. Your saved rules and prior updates stay private on this phone. Deleting your account also removes these local records.")
    }
}
...
private struct MetricPrototypeDetail: View {
    ...
    var body: some View {
            ...
            if let agreement {
                ...
                Section("Your private updates") {
                    ...
                    ForEach(agreement.proofs.reversed()) { proof in
                        Text("Update \(proof.revision): \(MetricPrototypeDisplay.state(proof.draft.state))\(proof.draft.value.map { " · \(MetricPrototypeDisplay.value($0, format: agreement.terms.draft.format))" } ?? ""). Replaces your previous update.")
                    }
                    if agreement.proofs.isEmpty { Text("No update saved. Missing data does not mean failure.") }
                }
                    ...
                    Section("Replace your fictional update") {
                        ...
                        Button("Save private update") {
                            ...
                            do {
                                ...
                                requestID = UUID(); message = "Your fictional update is saved. The outcome remains unresolved."
                            } catch { message = error.localizedDescription }
                        }.disabled(proof == nil)
                    }
                }
                Section {
                    if agreement.exitedAt != nil { Text("You left this practice record. No further updates can be added.") }
        ...
        } message: { Text("Leaving stops new updates. It does not create a win, loss or final result.") }
    }
}

enum MetricPrototypeDisplay {
    static func state(_ state: MetricPrototypeProofDraft.State) -> String {
        switch state { case .observed: "fictional distance"; case .missing: "no data"; case .unavailable: "not available" }
    }
