...
/// iPhone-side bounded reader for V1 Apple Watch steps. The Watch contributes
/// only through Apple Health on this iPhone. No sample, source, device, route,
/// or history data leaves this reader.
@MainActor
final class ChallengeHealthAppleWatchStepsReader: ChallengeHealthStore {
  private let querying: any ChallengeHealthAppleWatchStepsQuerying
  private var activeScope: ChallengeHealthAppleWatchStepsScope?
  private var ledger = ChallengeHealthAppleWatchStepsLedger()
  private var generation: UInt64 = 0
  private var attempt: UInt64 = 0

  init(healthStore: HKHealthStore = HKHealthStore()) {
    querying = ChallengeHealthHealthKitStepsQueryCoordinator(healthStore: healthStore)
  }

  init(querying: any ChallengeHealthAppleWatchStepsQuerying) {
    self.querying = querying
  }

  func read(_ request: ChallengeHealthReadRequest) async -> ChallengeHealthStoreOutcome {
    guard request.binding.metric == .steps else { return .unavailable(.queryFailed) }
    guard querying.isHealthDataAvailable else { return .unavailable(.protectedDataUnavailable) }
    let scope = ChallengeHealthAppleWatchStepsScope(request: request)
    ...
    do {
      ...
      let bounded = try await querying.boundedSteps(in: interval)
      try Task.checkCancellation()
      guard isCurrent(ticket, scope: scope) else { return .unavailable(.cancelled) }
      let changes = try await querying.anchoredStepChanges(in: interval, after: ledger.anchor)
      try Task.checkCancellation()
      guard isCurrent(ticket, scope: scope) else { return .unavailable(.cancelled) }
  ...
  private func begin(_ scope: ChallengeHealthAppleWatchStepsScope) -> (UInt64, UInt64)? {
    ...
    if activeScope != scope {
      activeScope = scope
      ledger = ChallengeHealthAppleWatchStepsLedger()
  ...
  private func isCurrent(_ ticket: (UInt64, UInt64),
                         scope: ChallengeHealthAppleWatchStepsScope) -> Bool {
...
private struct ChallengeHealthAppleWatchStepsScope: Equatable {
  let actorID: UUID
...
struct ChallengeHealthAppleWatchStepsLedger {
  ...
  mutating func merge(boundedRecords: [WeeklySourceRecord], additions: [WeeklySourceRecord],
    ...
    -> ChallengeHealthAppleWatchStepsMergedSnapshot {
    ...
    anchor = nextAnchor
    return ChallengeHealthAppleWatchStepsMergedSnapshot(
      records: records,
...
struct ChallengeHealthAppleWatchStepsMergedSnapshot {
...
@MainActor
protocol ChallengeHealthAppleWatchStepsQuerying: AnyObject {
  var isHealthDataAvailable: Bool { get }
  func boundedSteps(in interval: DateInterval) async throws -> [WeeklySourceRecord]
  func anchoredStepChanges(in interval: DateInterval, after anchor: HKQueryAnchor?) async throws
    -> ChallengeHealthAppleWatchAnchoredStepChanges
}

@MainActor
struct ChallengeHealthAppleWatchAnchoredStepChanges {
...
@MainActor
private final class ChallengeHealthHealthKitStepsQueryCoordinator: ChallengeHealthAppleWatchStepsQuerying {
  ...
  func anchoredStepChanges(in interval: DateInterval, after anchor: HKQueryAnchor?) async throws
    -> ChallengeHealthAppleWatchAnchoredStepChanges {
    let continuation = ChallengeHealthQueryContinuation<ChallengeHealthAppleWatchAnchoredStepChanges>()
    let query = HKAnchoredObjectQuery(
      type: HKQuantityType(.stepCount), predicate: Self.predicate(for: interval),
      ...
      let additions = added ?? []
      let deleted = deleted ?? []
      continuation.finish(.init(.success(ChallengeHealthAppleWatchAnchoredStepChanges(
  ...
  private func samples(predicate: NSPredicate, limit: Int) async throws -> [HKSample] {
    ...
    let query = HKSampleQuery(
      sampleType: HKQuantityType(.stepCount), predicate: predicate, limit: limit,
  ...
  nonisolated private static func record(from sample: HKSample) -> WeeklySourceRecord? {
    ...
    return WeeklySourceRecord(
      id: sample.uuid, metric: .steps, start: sample.startDate, end: sample.endDate,
