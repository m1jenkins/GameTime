...
def open_actor(number):
    ...
    assert APP.is_dir(),'Build the Debug app first; see docs/BETA_LOCAL_PREVIEW.md'
...
def main():
    ...
    serve=sub.add_parser('serve');serve.add_argument('--no-open',action='store_true')
    ...
    if args.command=='serve':
        ...
        try:
            smoke.setup()
            value=json.loads(support.MANIFEST.read_text());value['controllerKind']='preview'
