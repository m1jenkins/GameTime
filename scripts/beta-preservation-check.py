...
head=subprocess.check_output(['git','--no-optional-locks','-C',str(ORIGINAL),'rev-parse','HEAD'],text=True).strip()
status=subprocess.check_output(['git','--no-optional-locks','-C',str(ORIGINAL),'status','--porcelain=v1','--untracked-files=all'])
failures=[]
