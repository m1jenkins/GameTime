#!/usr/bin/env bash
# Disposable two-actor demonstration. The endpoint is deliberately loopback,
# and the entire fixture/admission/agreement transaction is rolled back.
set -euo pipefail
...
psql "postgresql://postgres:postgres@127.0.0.1:${checkpoint_db_port}/postgres" \
  --no-psqlrc --set ON_ERROR_STOP=1 --file scripts/examples/duel-agreement.sql
