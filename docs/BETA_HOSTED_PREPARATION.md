...
| Invitation links | Debug `gametime-beta://challenge-invite/<64-hex-token>`; only opaque locator before sign-in | Approve HTTPS host, exact application identifier, `/challenge-invite/*`, [association template](release/beta/apple-app-site-association.json.template), entitlement and provisioned binary. No associated-domain entitlement or published host exists. Native HTTPS invitation parsing/routing still needs implementation and testing; an AASA file alone does not add it. |
