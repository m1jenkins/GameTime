...
| Support and reviews | `scripts/beta-operator.py --owned-project gametime-finish-b7 status`; use the exact scoped commands in `docs/BETA_OPERATIONS_LOCAL.md` for fictional cases. Report/review capability and conflict/expiry checks are implemented. | Accountable legal/support identity, monitored route, named operators and coverage. Wire approved support text/URLs and prove delivery only after permission to send a real test message. Local reports are not a monitored support service. |
