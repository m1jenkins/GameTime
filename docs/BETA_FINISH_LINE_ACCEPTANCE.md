# Beta finish-line acceptance — isolated b7 execution

Authority: D134 and [Beta implementation plan](BETA_IMPLEMENTATION_PLAN.md).
...
- Original read-only source: `/Users/user/Documents/GitHub/GameTime`, HEAD `577bc321e72750976e2e8027680387707070b0e3`.
- Inherited checkpoint `c114078`: eight modified tracked Markdown files, both new plans, all September 7 design directories and untracked Lavish review material. 101 files / 51,192,467 bytes, every SHA-256 verified. Authorship uncertain; original design status preserved.
- [Source manifest](evidence/beta-finish-line-b7/inherited-sources.json), [exact original status](evidence/beta-finish-line-b7/original-status.txt), [pre-existing containers](evidence/beta-finish-line-b7/preexisting-containers.txt), [database identity](evidence/beta-finish-line-b7/original-database-identity.txt), [pre-existing Simulators](evidence/beta-finish-line-b7/preexisting-simulators.txt).
- Existing containers, database, and Simulators are not task resources. No source reset, stash, hardlink, push or primary-checkout mutation.
- The memory helper found distinct real AGENTS/CLAUDE files. Reconciled conventions into AGENTS and installed its canonical CLAUDE pointer; inherited contents remain in the separate checkpoint.
...
- M2: `psql postgresql://postgres:postgres@127.0.0.1:58322/postgres -X -v ON_ERROR_STOP=1 -f supabase/tests/490_challenge_steps.test.sql` (30 assertions), then `491_challenge_boundaries.test.sql` (18). Both rollback-only. Logs `/tmp/gametime-finish-b7-evidence/m2-{steps,boundaries}-tests.log`; no `not ok`/errors in final runs.
- M2 fixes from executed failures: parenthesized interval CASE for PL/pgSQL parsing; disambiguated projection/result variable; source constraint test checks its actual constraint rejection; deferred safe-exit trigger uses its owner privileges, since deferred execution happens outside the mutation function.
- M1 screen verified using explicit `xcrun simctl launch 72A3249A-2DE0-4695-AF41-DCD2743B4666 com.mjenkins.gametime.staging --health-source-investigation`; MCP launch did not forward the requested argument. Actual UI displayed private investigation and Begin private session.

M2 reversible resource choices: maximum 10 open drafts per creator, 30 pending direct entrants, 128 correction revisions, canonical steps/targets up to 1,000,000,000. One new-domain runtime mutex serializes local Beta mutations; participant profile locks use UUID order. This conservative small-beta design needs throughput assessment before hosted operation. Unselected/exited/blocked membership gets an own-only projection; no comparative historical series is exposed.

- First committed portable run (`b8b708f`, `/tmp/gametime-weekly-verify.BsjK3yuw`) executed 3,624 SQL assertions/74 files, 835 Deno tests, 113 Swift core tests. One historical SQL assertion failed: wrong-consent fixture called a wall-clock RPC after its fixed September 7 join cutoff, receiving `weekly_join_unavailable`. Added a deterministic test-only clock wrapper to test the same wrong-digest implementation while its fixture is open; no weekly production behavior/migration changed. This failed run is not acceptance; rerun required.
