...
## Next step: create one clean alpha candidate

Do this before adding another product feature.

1. Preserve the optional M8.4 push and reaction working-tree patch outside the alpha candidate. Do not merge it into the functional path.
2. Start the alpha candidate from current `main`.
3. Confirm whether `main` already contains the App Attest ECDSA fallback and current staging App IDs. Port only those required fixes if they are absent.
4. Leave D76 adjudication operations, APNs delivery, comeback reactions, and other optional work out of the candidate.
5. Run the complete database, Deno, Swift package, product, configuration, and conformance test matrix.
6. Obtain one complete green CI run for the exact candidate revision.
7. Merge or push only after explicit approval.
...
## Prove the hosted challenge lifecycle
